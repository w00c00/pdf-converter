...
3. 输入�?   - **Name**: `CLOUDCONVERT_API_KEY`
   - **Value**: 你的API密钥

### 步骤7: 部署

1. 点击页面底部�?**"Deploy the stack"** 按钮
2. 等待部署完成（约1-2分钟�?
### 步骤8: 验证
...
### 环境变量
- `CLOUDCONVERT_API_KEY`: 可选，不填则使用模拟模�?- `PORT`: 容器内端口，保持3456
- `BASE_URL`: 访问地址
...
1. 查看容器日志
2. 常见问题�?   - **端口占用**: 3456端口被占用，修改端口映射
   - **镜像下载失败**: 检查网络连�?   - **权限问题**: 检查volume权限
...
项目中的配置文件�?- `portainer-stack.yml` - Portainer Stack配置

...
## 📋 部署前准�?
### 你需要提供的信息�?1. **Portainer管理地址**（例如：http://192.168.1.100:9000�?2. **API Token** 或用户名/密码
...
2. 点击 **"+ Add stack"**
3. 选择 **"Web editor"**
4. 复制 `portainer-stack.yml` 的内容粘贴进�?
### 步骤5: 配置环境变量
...
1. 查看容器状态（应该显示Running�?2. 查看日志确认无错�?3. 访问 http://服务器IP:3000
...
在Portainer中：
1. 点击右上角用户头�?2. 选择 **"My account"** �?**"我的账户"**
...
curl -X POST \
  ...
  -d '{
    "Name": "pdf-converter",
    "StackFileContent": "...(portainer-stack.yml的内�?..."
  }' \
...
2. 检查端口映射：
   ```bash
   docker ps
   # 确认PORTS列显�?0.0.0.0:3000->3000/tcp
   ```

### API密钥不生�?
1. 确认.env文件或环境变量配置正�?2. 重启容器让配置生�?
---

## 📊 监控和维�?
### 查看状�?
```bash
# 查看容器状�?docker ps | grep pdf-converter
...
# 最�?00�?docker logs --tail 100 pdf-converter
```
